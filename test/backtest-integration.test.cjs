...
test('historical fetch flows through all algorithms into versioned learning without touching live state', async () => {
  ...
  try {
    ...
    assert.equal(result.strategies.length, 23);
    ...
    assert.equal(result.validation.strategies.length, 23);
    ...
    assert.equal(events.filter((event) => event.phase === 'strategy-completed').length, 23);
