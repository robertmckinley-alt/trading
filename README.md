...
3. a nine-bot, paper-only NQ strategy research network plus nine isolated ORB forward experiments
...
- one persistent Databento Live API stream shared by all nine watchers and the ORB supervisor (no Historical API polling or fallback)
...
- the main page shows all nine primary strategy accounts plus outcomes, but Vercel needs a live status bridge to read the VPS watchers
