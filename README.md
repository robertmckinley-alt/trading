...
3. an eight-bot, paper-only NQ strategy research network plus nine isolated ORB forward experiments
...
- one persistent Databento Live API stream shared by all eight watchers and the ORB supervisor (no Historical API polling or fallback)
- eight isolated paper strategy watchers sharing that one live stream, including the baseline ORB close-confirmation strategy and the DMC market-open translation
...
- a shared `$2,500` simultaneous open-risk cap across all eight primary paper accounts plus smaller correlated-strategy family caps, enforced before a strategy can reserve a new plan
...
- the main page shows all eight primary strategy accounts plus outcomes, but Vercel needs a live status bridge to read the VPS watchers
