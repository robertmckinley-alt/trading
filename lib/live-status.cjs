...
function parseLogEvents(logText) {
  ...
  return starts.map((match, index) => {
    ...
    if (header === 'live tick' || candleMatch || reasonMatch) {
      return {
        type: 'tick',
        at: match[1],
...
function buildNineAmStrategy(config, state, logText, pid, sourceMode) {
  ...
  return {
    ...
    watcher: {
      ...
      staleStatusHint: !isRunning && liveStatus.lastCandle ? 'Status script may be stale or detached from the real process state.' : null
    },
    live: {
...
function normalizeRemoteStrategy(strategy, logUpdatedAt) {
  if (!strategy || strategy.mode !== 'live-watcher') {
    return strategy;
  }

  const latestErrorAt = Date.parse(strategy.live?.latestError?.at || '');
  const logUpdatedTime = Date.parse(logUpdatedAt || '');
  const hasRecoveredTick =
    Boolean(strategy.live?.lastCandle) &&
