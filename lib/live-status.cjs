...
const { getPortfolioRiskSnapshot } = require('./portfolio-risk.cjs');
...
function summarizeTradesForDate(trades, date) {
  ...
  return {
    ...
    tradesList: closedTrades.map((trade) => ({
      ...
      signalContext: trade.signalContext || null,
...
function buildLiveStrategy({
  ...
  const learning = state?.learning || state?.live?.adaptive?.learning || null;
  const adaptive = state?.live?.adaptive
    ? { ...state.live.adaptive, learning: undefined }
    : null;
  ...
  return {
    ...
    live: {
      ...
      adaptive,
      learning,
      researchContext: state?.live?.researchContext || null,
      researchCouncil: state?.live?.researchCouncil || null
    },
    setupSummary,
    research: research
...
function getLocalStrategySnapshots() {
  ...
  const snapshots = STRATEGIES.map((definition) => {
    const strategyConfig = definition.slug === 'mgc-open-ema12' ? require('./gold-open-ema.cjs').config(config) : definition.slug === 'nq-vwap-stretch-reversion' ? { ...config, ...require('./vwap-stretch.cjs').ACCOUNT } : config;
    const files = runtimeFilesForStrategy(process.cwd(), definition.slug);
    const state = hydrateState(safeReadJson(files.statePath), strategyConfig);
  ...
  return {
    ...
    orbForward: require('./orb-forward.cjs').snapshot(process.cwd()),
    generatedAt: new Date().toISOString(),
    portfolioRisk: getPortfolioRiskSnapshot(process.cwd(), config),
