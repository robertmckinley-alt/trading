const { synchronizeStrategyLearning } = require('./strategy-learning.cjs');
...
function riskGuardBot(config, state, market, performance, learning = null) {
  ...
  const multiplier = Math.max(0.5, Math.min(
    1,
    market.riskMultiplier,
    performance.riskMultiplier,
    Number(learning?.adjustment?.riskMultiplier ?? 1)
  ));
...
function evaluateAdaptiveBots(candles, config, state) {
  ...
  const learning = synchronizeStrategyLearning(state.trades, {
    previous: state.learning,
    strategySlug: config.strategySlug
  });
  const risk = riskGuardBot(config, state, market, performance, learning);
  return {
