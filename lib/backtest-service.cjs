...
const { buildBacktestValidation } = require('./backtest-validation.cjs');
...
async function executeBacktest(options = {}) {
  ...
  const patterns = require('./pattern-matching.cjs').build(historical.candles, result);
  result.patternMatching = patterns.report;
  if (!env.VERCEL && !options.cacheDir) require('./pattern-matching.cjs').saveModel(process.cwd(), 'NQ', patterns.model);
  result.validation = buildBacktestValidation(result, { ...provenance, config, definitions: BACKTEST_STRATEGIES });
  ...
  // Reuse the candles already loaded by this worker. No discovery data downloads.
  if (!env.VERCEL && options.startYear === 2025) {
    try {
      result.priceActionQualification = require('./price-action-qualification.cjs').qualify(result, config, process.cwd());
    } catch (error) {
      result.priceActionQualification = { status: 'failed', message: error.message, reviews: [] };
    }
    try {
      result.discovery = require('./orb-discovery.cjs').runDaily({ root: process.cwd(), candles: historical.candles,
        config, report: result, now, onProgress: options.onProgress });
    } catch (error) { result.discovery = { status: 'failed', message: error.message }; }
  }
  return result;
}
