...
const { fetchDatabentoHistoricalCandles, historicalWindow } = require('./historical-data.cjs');
...
async function fetchRemoteBacktest(url, days, env = process.env, fetchImpl = fetch) {
...
async function executeBacktest(options = {}) {
  ...
  const days = historicalWindow(options.days || 60).days;
  if (!env.DATABENTO_API_KEY) {
    ...
    return Promise.any(urls.map((url) => fetchRemoteBacktest(url, days, env, options.fetchImpl || fetch)));
  }
  const historical = await fetchDatabentoHistoricalCandles({ days, env, fetchImpl: options.fetchImpl });
...
module.exports = { backtestConfigured, executeBacktest, remoteBacktestUrls };
