...
function normalizeStrategyConfig(rawConfig) {
  ...
  return {
    provider: String(live.provider || process.env.LIVE_DATA_PROVIDER || 'mock'),
    ticker: String(live.ticker || process.env.LIVE_DATA_TICKER || 'NQU6'),
    mockCsvPath: String(live.mockCsvPath || process.env.LIVE_DATA_MOCK_CSV || path.join(process.cwd(), 'examples', 'sample-nq-1m.csv')),
    apiKeyEnv: String(live.apiKeyEnv || process.env.LIVE_DATA_API_KEY_ENV || 'LIVE_DATA_API_KEY'),
