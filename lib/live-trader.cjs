...
function fetchDatabentoLiveCandles(strategyConfig) {
  ...
  if (strategyConfig.ticker === 'MGC.v.0' && (payload.symbol !== 'MGC.v.0' || payload.dataset !== 'GLBX.MDP3' || payload.schema !== 'ohlcv-1m')) throw new Error('Gold cache identity mismatch: expected MGC.v.0 GLBX.MDP3 ohlcv-1m');
  ...
  for (const candle of candles) {
    ...
    if (!existing) {
      groups.set(key, {
        ...
        volume: Number(candle.volume || 0),
        samples: 1,
        times: [Date.parse(candle.timestamp)]
      });
      continue;
    }
    ...
    existing.samples += 1;
    existing.times.push(Date.parse(candle.timestamp));
  }
  return [...groups.values()].filter((candle) => candle.samples === intervalMinutes
    && candle.times.every((time, i) => Number.isFinite(time) && time % 60_000 === 0
      && (i === 0 || time - candle.times[i - 1] === 60_000)))
    .map(({ times, ...bar }) => bar);
}
...
function detectHtfSessionSweepSignal(candles, rawConfig, state) {
  ...
  const confirmations = [...fiveMinuteBars].reverse().filter((bar) => {
    ...
    return bodyFraction >= rules.minimumConfirmationBodyFraction
      && bar.close > asia.low && bar.close < asia.high
      && (side === 'long' ? bar.close > bar.open : bar.close < bar.open);
  });
  if (!confirmations.length) return { found: false, reason: 'Waiting for a completed five-minute rejection back inside the Asia range', metadata: { tradingDate } };
  let confirmation = null;
  let bos = null;
  for (const candidate of confirmations) {
    const firstBreak = entryCandles.find(c => Date.parse(c.timestamp) > Date.parse(candidate.endTimestamp)
      && (side === 'long' ? c.close > candidate.high : c.close < candidate.low));
    if (firstBreak?.timestamp === latest.timestamp) { confirmation = candidate; bos = firstBreak; break; }
  }
  if (!bos) return { found: false, reason: 'Waiting for a fresh one-minute structure break after five-minute rejection', metadata: { tradingDate } };
...
function detectSignalFromCandles(candles, rawConfig, state) {
  if (rawConfig.strategySlug === 'mgc-open-ema12') {
    const s = require('./gold-open-ema.cjs').detect(candles, rawConfig, state);
    if (!s.found) return s;
    return {found:true,triggerTimestamp:s.triggerTimestamp,metadata:s,rangeSummary:[],sessionRanges:{},
      setup:{...buildResearchSetup({rawConfig,tradingDate:s.date,session:'Gold NY opening',side:s.side,entry:s.entry,stop:s.stop,targets:[],
        thesis:'Gold-only opening candle versus five-minute EMA12; mechanical trailing assumption, unvalidated.',
        entryModel:'gold-open-ema12',gapType:'ema-direction',entryTimeframe:'M5',activationTime:'09:35 New York',
        referenceSessions:['ny-open'],liquidityPool:'opening-range',liquidityLabel:'EMA12',drawOnLiquidity:['trailing-momentum']}),
        execution:'next-bar-market',trailingModel:'gold-five-minute-1r',flattenAt:s.flattenAt,
        signalAvailableAt:new Date(Date.parse(s.triggerTimestamp)+60000).toISOString(),orderExpiresAt:new Date(Date.parse(s.triggerTimestamp)+180000).toISOString()}};
  }
  if (rawConfig.strategySlug === require('./vwap-stretch.cjs').SLUG) {
    const s = require('./vwap-stretch.cjs').detect(candles, rawConfig, state);
    if (!s.found) return s;
    return {found:true,triggerTimestamp:s.triggerTimestamp,metadata:s,rangeSummary:[],sessionRanges:{},
      setup:{...buildResearchSetup({rawConfig,tradingDate:s.date,session:'Cash VWAP',side:'long',entry:s.entry,stop:s.stop,targets:s.targets,
        thesis:'Video-inspired long mean reversion at 2 ATR below candle-based session VWAP; mechanics independently specified.',
        entryModel:'vwap-stretch-reversion',gapType:'vwap-stretch',entryTimeframe:'M1',activationTime:'Full cash session after warmup',
        referenceSessions:['cash-session'],liquidityPool:'session-vwap',liquidityLabel:String(s.vwap),drawOnLiquidity:['session-vwap']}),
        execution:'next-bar-market',exitAllAtTarget:true,flattenAt:s.flattenAt,
        signalAvailableAt:new Date(Date.parse(s.triggerTimestamp)+60000).toISOString(),orderExpiresAt:new Date(Date.parse(s.triggerTimestamp)+120000).toISOString()}};
  }
