...
function normalizeStrategyConfig(rawConfig) {
  ...
  const resolvedProvider = !requestedProvider || requestedProvider === 'auto'
    ? (hasDatabentoKey ? 'databento' : 'mock')
    : requestedProvider;
  return {
    ...
    fetchLookbackBars: Number(live.fetchLookbackBars || process.env.LIVE_DATA_FETCH_LOOKBACK_BARS || 2880),
    pollIntervalMs: Number(live.pollIntervalMs || process.env.LIVE_DATA_POLL_INTERVAL_MS || 60000),
    fetchTimeoutMs: Number(live.fetchTimeoutMs || process.env.LIVE_DATA_FETCH_TIMEOUT_MS || 45000),
    sessionWindows: {
      asia: {
...
function isoUtcMinutesAgo(minutes) {
  const date = new Date(Date.now() - (minutes * 60 * 1000));
  date.setUTCSeconds(0, 0);
  return date.toISOString();
}

function isoUtcMinuteBoundary(offsetMinutes = 0) {
  const date = new Date(Date.now() + (offsetMinutes * 60 * 1000));
  date.setUTCSeconds(0, 0);
  return date.toISOString();
}

function isoUtcFromTimestamp(value) {
  const date = new Date(value);
  date.setUTCSeconds(0, 0);
  return date.toISOString();
}

function normalizeIsoLikeTimestamp(value) {
  const raw = String(value || '').trim();
  if (!raw) {
    return null;
  }

  const normalizedSpace = raw.replace(' ', 'T');
  const normalizedFraction = normalizedSpace.replace(
    /\.(\d{3})\d+Z$/i,
    '.$1Z'
  );
  const date = new Date(normalizedFraction);
...
function parseDatabentoRetryEnd(error) {
  const message = String(error?.stderr || error?.message || error || '');
  const patterns = [
    /available up to '([^']+)'/i,
    /end time before ([0-9T:+\-.]+Z)\.?/i
  ];
  ...
  for (const pattern of patterns) {
    ...
    const retryAt = normalizeIsoLikeTimestamp(match[1]);
    if (!retryAt) {
      continue;
    }

    // Databento `end` is exclusive. Step back one full minute from the
    // provider-reported boundary to avoid re-requesting the failing edge.
...
function fetchDatabentoRange(strategyConfig, start, end) {
  const helperPath = path.join(__dirname, 'fetch_databento_ohlcv.py');
  const args = [
    helperPath,
    '--key-env', strategyConfig.apiKeyEnv,
    '--dataset', strategyConfig.dataset,
    '--symbol', strategyConfig.ticker,
    '--schema', strategyConfig.schema,
    '--stype-in', strategyConfig.stypeIn,
    '--start', start,
    '--end', end,
    '--limit', String(Math.max(strategyConfig.fetchLookbackBars, strategyConfig.lookbackBars))
  ];

  const raw = execFileSync(strategyConfig.pythonBin, args, {
    cwd: process.cwd(),
    encoding: 'utf8',
    env: process.env,
    timeout: strategyConfig.fetchTimeoutMs,
...
function fetchDatabentoCandles(strategyConfig) {
  ...
  const delayedEndMinutes = 10;
  const fetchLookbackBars = Math.max(strategyConfig.fetchLookbackBars, strategyConfig.lookbackBars);
  const lookbackMinutes = Math.max(fetchLookbackBars + 120 + delayedEndMinutes, 2880);
  let end = isoUtcMinuteBoundary(-delayedEndMinutes);
  let start = isoUtcMinutesAgo(lookbackMinutes);
  let payload;

  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      payload = fetchDatabentoRange(strategyConfig, start, end);
      break;
    } catch (error) {
      const retryEnd = parseDatabentoRetryEnd(error);
      if (!retryEnd) {
        throw error;
      }
      ...
      start = isoUtcFromTimestamp(endDate.getTime() - (lookbackMinutes * 60 * 1000));
      if (attempt === 2) {
        throw error;
      }
    }
  ...
  if (!candles.length) {
    throw new Error(`No Databento candles returned for ${strategyConfig.ticker}`);
  }
  return {
    source: `databento:${strategyConfig.dataset}:${strategyConfig.ticker}`,
    candles,
    metadata: {
      provider: 'databento',
      ...
      note: 'Databento historical OHLCV polling for live minute-bar watcher',
...
async function fetchLiveCandles(rawConfig) {
  ...
  if (strategyConfig.provider === 'databento') {
    return fetchDatabentoCandles(strategyConfig);
  }
