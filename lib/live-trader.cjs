...
function normalizeStrategyConfig(rawConfig) {
  ...
  const hasDatabentoKey = Boolean(process.env[String(live.apiKeyEnv || process.env.LIVE_DATA_API_KEY_ENV || 'DATABENTO_API_KEY')]);
  return {
    provider: String(live.provider || process.env.LIVE_DATA_PROVIDER || (hasDatabentoKey ? 'databento' : 'mock')),
    ...
    apiKeyEnv: String(live.apiKeyEnv || process.env.LIVE_DATA_API_KEY_ENV || 'DATABENTO_API_KEY'),
...
function fetchDatabentoCandles(strategyConfig) {
  ...
  const start = isoUtcMinutesAgo(Math.max(strategyConfig.lookbackBars + 120, 1440));
  const raw = execFileSync(
    strategyConfig.pythonBin,
    [
      helperPath,
      ...
      '--start', start,
      '--limit', String(strategyConfig.lookbackBars)
    ],
    {
      cwd: process.cwd(),
      encoding: 'utf8',
