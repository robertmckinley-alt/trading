...
function normalizeRecord(record) {
  const normalized = {
    timestamp: timestamp(record.ts_event ?? record.timestamp),
...
function parseDatabentoJson(raw) {
  ...
  if (candles.length > MAX_CANDLES) throw new Error(`Historical response exceeded the ${MAX_CANDLES.toLocaleString()} candle safety limit.`);
  return candles;
}

function historicalWindow(days = 60, now = new Date()) {
  const safeDays = Math.max(10, Math.min(90, Math.round(Number(days) || 60)));
...
async function fetchDatabentoHistoricalCandles(options = {}) {
  ...
  const response = await fetchImpl(`${HISTORICAL_URL}?${params}`, {
  ...
  const candles = parseDatabentoJson(await response.text());
...
module.exports = { fetchDatabentoHistoricalCandles, historicalWindow, parseDatabentoJson };
