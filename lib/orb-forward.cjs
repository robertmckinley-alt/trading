...
const DEFINITIONS = BACKTEST_STRATEGIES.filter(s => s.slug.startsWith('nq-15m-orb-') && s.forwardPaperEligible !== false);
const ORB_FORWARD_RULES_VERSION = 'orb-forward-paper-v2-cash-session';
...
function version(config, rulesVersion = ORB_FORWARD_RULES_VERSION) {
  const rules = {
    version: rulesVersion,
    instrument: { tickSize: config.tickSize, tickValueUsd: config.tickValueUsd },
    costs: { commissionPerContractUsd: config.commissionPerContractUsd, slippageTicks: config.slippageTicks },
    openingRangeClose: config.live?.openingRangeClose || {},
    definitions: DEFINITIONS.map(({ slug, researchRules }) => ({ slug, researchRules: researchRules || null })),
    execution: 'one-contract-next-unseen-minute-open-v1'
  };
  const hash = createHash('sha256').update(JSON.stringify(rules));
...
function attachCandidates(state, candidates, codeHash, now = Date.now()) {
  for (const c of candidates.slice(0, 7)) {
    if (c.status !== 'paper-candidate' || c.implementationHash !== codeHash || !c.frozenConfig
      || state.accounts.some(a => a.slug === c.slug)) continue;
    state.accounts.push({ slug: c.slug, name: `Discovery: ${c.name}`, baseSlug: c.baseSlug,
      experimentId: c.id, implementationHash: c.implementationHash, frozenConfig: c.frozenConfig,
      startedAt: new Date(now).toISOString(), trades: [], attempts: [], active: null, consumedDay: null,
      netPnlUsd: 0, equityPeakUsd: 0, maxDrawdownUsd: 0, markPeakUsd: 0, markDrawdownUsd: 0 });
  }
}
...
function advance(state, incoming, history, config, now = Date.now(), deps = {}) {
  ...
  for (const candle of pending) {
    ...
    for (const account of state.accounts) {
      ...
      const cashSession = require('./orb-session.cjs').session(active.day);
      const closeMinute = cashSession.closeMinute || 960;
      // Resolve New York's offset from the signal day, including DST.
      const anchor = Date.parse(active.observedAt);
      const flattenAt = new Date(anchor - (parts(anchor).minute * 60000 + anchor % 60000) + closeMinute * 60000).toISOString();
      const delayedClose = at >= Date.parse(flattenAt);
      if (delayedClose) active.dataGap = true;
      ...
      const result = replay(active.plan, active.candles, account.frozenConfig || config, { researchFixedContracts: 1,
        flattenAt, closeOpenAtEnd: p.minute >= closeMinute - 1 || p.date !== active.day });
      if (result.exitReason === 'end-of-data close') result.exitReason = 'scheduled cash-session close';
      active.result = result;
      if (result.filledAt) account.consumedDay = active.day;
      if (result.status === 'closed') {
        const trade = { ...toJournalTrade(active.plan, result), id: `${account.slug}-${active.observedAt}`, evidenceType: 'forward-paper', rulesVersion: active.rulesVersion || state.version, observedAt: active.observedAt, dataQuality: active.dataGap ? 'gap-review-required' : 'complete' };
    ...
    if (p.minute % 5 === 4 && p.minute >= 589 && p.minute <= 689 && now - (at + 60000) < 60000) {
      ...
      for (const account of state.accounts) {
        const pattern = require('./price-action-patterns.cjs').SLUGS.includes(account.baseSlug || account.slug);
        if (!pattern && (p.minute < 599 || p.minute % 15 !== 14)) continue;
        if (account.implementationHash && account.implementationHash !== deps.implementationHash) {
          account.lastDecision = { at: state.heartbeat, reason: 'Frozen research implementation changed; new entries paused for review.' };
          continue;
        }
        if (account.active || account.consumedDay === p.date || account.attempts.some(a => a.candleAt === candle.timestamp)) continue;
        const accountConfig = account.frozenConfig || config;
        const signal = detect(available.slice(-2500), { ...accountConfig, strategySlug: account.baseSlug || account.slug, orbResearchContext: context }, { trades: [] });
        ...
        const plan = build(signal, accountConfig);
        ...
        account.active = { plan, day: p.date, observedAt, rulesVersion: account.experimentId || state.version, candles: [], dataGap: false, result: null };
      }
    }
...
function snapshot(root) {
  ...
  try {
    ...
    return { version: state.version, startedAt: state.startedAt, heartbeat: state.heartbeat, latestCandleAt: state.latestCandleAt,
      supervisedAccounts: state.accounts.length, lastError: state.lastError || null,
      ...
      accounts: state.accounts.map(a => ({ slug: a.slug, name: a.name, netPnlUsd: a.netPnlUsd,
        startedAt: a.startedAt || state.startedAt, experimentId: a.experimentId || null,
        ...
        trades: a.trades, lastDecision: a.lastDecision, active: a.active ? {
          side: a.active.plan.setup.side, stop: a.active.plan.setup.stop,
          status: a.active.result?.status || 'pending',
          entry: a.active.result?.filledEntryPrice ?? null,
          filledAt: a.active.result?.filledAt || null,
          observedAt: a.active.observedAt,
          markedAt: a.active.result?.filledAt ? a.active.candles.at(-1)?.timestamp || null : null,
          dataGap: Boolean(a.active.dataGap),
          unrealizedPnlUsd: a.active.result?.unrealizedPnlUsd || 0
        } : null })) };
  } catch { return { status: 'state-unavailable', accounts: [] }; }
}
module.exports = { DEFINITIONS, atomic, version, createState, mergeCandles, readWarmup, advance, snapshot, attachCandidates };
