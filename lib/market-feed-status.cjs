...
function clock(now = new Date()) {
  ...
  return { regularMarketClosed: closed, orbEntryWindow: !['Sat', 'Sun'].includes(p.weekday) && minute >= 599 && minute <= 691, timeZone: 'America/New_York', calendar: 'Regular weekly hours; exchange holidays may differ.' };
}
