...
function runStrategyBacktest(candles, rawConfig, definition, dependencies = {}) {
  ...
  for (const [dayIndex, [date, records]] of days.entries()) {
    ...
    if (isOrb(definition.slug)) {
      try {
        const researchPlan = buildPlan(signal, config, createEmptyState(config));
        const researchReplay = replay(researchPlan, future, config);
        ...
        if (/drawdown guard|No trade: minimum 1-contract risk/.test(error.message)) researchRejected += 1;
        else throw error;
      }
    }
  ...
  return {
    ...
    ...(isOrb(definition.slug) ? { research: {
...
function runAllBacktests(candles, config, options = {}) {
  ...
  return {
    ...
    evidenceType: 'historical-simulated',
    methodology: 'Completed-candle signals; explicit ORB next-bar market execution with adverse slippage and stop gaps. Other strategies retain price-touch entries. Retrospective research, not untouched forward evidence.',
