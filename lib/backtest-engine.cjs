...
function checkpointsForDay(slug, records) {
  if (slug === 'mgc-open-ema12') return records.filter(r => r.minute === 574);
  if (slug === 'nq-vwap-stretch-reversion') return records.filter(r => r.minute >= 584 && r.minute <= 957);
  if (require('./price-action-patterns.cjs').SLUGS.includes(slug)) return records.filter(r => r.minute >= 589 && r.minute <= 689 && r.minute % 5 === 4);
  ...
  if (slug === 'nq-dmc-market-open') {
    return cashEnd.filter((record) => record.minute >= 574 && record.minute <= 624 && record.minute % 5 === 4);
  }
...
function runStrategyBacktest(candles, rawConfig, definition, dependencies = {}) {
  if (definition.slug === 'nq-vwap-stretch-reversion') return require('./vwap-backtest.cjs').run(candles, rawConfig, definition, dependencies);
  ...
  const config = {
    ...rawConfig,
    ...(definition.slug === 'nq-vwap-stretch-reversion' ? require('./vwap-stretch.cjs').ACCOUNT : {}),
  ...
  const filterChecks = {};
  const signalDiagnostics = { checkpoints: 0, daysEvaluated: 0, noSignalDays: 0, reasons: {} };
  let signals = 0;
  let notFilled = 0;
  ...
  for (const [dayIndex, [date, records]] of days.entries()) {
    const cashSession = require('./orb-session.cjs').session(date);
    if (isOrb(definition.slug) && !cashSession.open) continue;
    ...
    let signal = null;
    let detectedAt = null;
    const checkpoints = checkpointsForDay(definition.slug, records);
    if (checkpoints.length) signalDiagnostics.daysEvaluated += 1;
    for (const checkpoint of checkpoints) {
      ...
      signalDiagnostics.checkpoints += 1;
      if (!candidate.found) {
        // Aggregate dynamic price/count text to keep long backtests bounded.
        const reason = String(candidate.reason || 'Unspecified rejection').replace(/\d+(?:\.\d+)?/g, '#');
        signalDiagnostics.reasons[reason] = (signalDiagnostics.reasons[reason] || 0) + 1;
      }
    ...
    if (!signal) { if (checkpoints.length) signalDiagnostics.noSignalDays += 1; continue; }
    signals += 1;
    ...
    const future = records
      .filter((record) => record.minute < (isOrb(definition.slug) ? cashSession.closeMinute : 960) && Date.parse(record.candle.timestamp) > detectionTime)
  ...
  return {
    ...
    ...(definition.researchRules ? { researchRules: definition.researchRules } : {}),
    ...(definition.forwardPaperEligible === false ? { forwardPaperEligible: false, researchStage: definition.researchStage } : {}),
    evidenceType: 'historical-simulated',
    signalDiagnostics,
    signals,
    notFilled,
