...
function checkpointsForDay(slug, records) {
  ...
  if (slug !== 'ema-20-60-momentum') return [cashEnd.at(-1)];
...
function runStrategyBacktest(candles, rawConfig, definition, dependencies = {}) {
  ...
  const config = {
    ...
    live: { ...(rawConfig.live || {}), maxSignalAgeBars: 2000, maxTradesPerDay: 1 }
  };
  ...
  for (const [, records] of days) {
    ...
    try {
      ...
      if (/drawdown guard|No trade: minimum 1-contract risk/.test(error.message)) break;
      throw error;
    }
  }
...
module.exports = { groupTradingDays, runAllBacktests, runStrategyBacktest };
