...
function reviewBacktestEvidence(trades = [], { enforceAccountDrawdown = true } = {}) {
  const total = performanceMetrics(trades);
  const { training, holdout } = splitChronologically(trades);
  const trainingMetrics = performanceMetrics(training);
  const holdoutMetrics = performanceMetrics(holdout);
  ...
  if (enforceAccountDrawdown && total.maxDrawdownUsd > 5000) redFlags.push('Historical maximum drawdown exceeds the account drawdown limit.');
  ...
  if (enoughTrades && profitable && (!enforceAccountDrawdown || total.maxDrawdownUsd <= 5000)) recommendation = 'ADVANCE TO FORWARD TEST';
  ...
  return {
    ...
    gates: {
      ...
      positiveHoldout: holdoutMetrics.expectancyUsd > 0,
      profitFactor: total.profitFactor === null ? total.netPnlUsd > 0 : total.profitFactor >= 1.2,
      ...(enforceAccountDrawdown ? { drawdown: total.maxDrawdownUsd <= 5000 } : {})
    }
  };
}
