...
// A bar must END before the cash open. The 09:00 bucket includes cash trading.
function preOpenHours(candles, tradingDate, maximumHourlyBars) {
  const open = Date.parse(timestampForWallClock(tradingDate, '09:30'));
  return completeBars(candles, 60)
    .filter(bar => Date.parse(bar.endTimestamp) + 60_000 <= open)
    .slice(-maximumHourlyBars);
}
...
function detectDmcMarketOpenSignal(candles, rawConfig, state = { trades: [] }) {
  ...
  const hourly = preOpenHours(candles, tradingDate, rules.maximumHourlyBars);
