...
function validateSetup(setup, config) {
  ...
  const modelRules = {
    'gold-open-ema12': { sweep: false, gapTypes: ['ema-direction'], timeframes: ['M5'] },
    'vwap-stretch-reversion': { sweep: false, gapTypes: ['vwap-stretch'], timeframes: ['M1'] },
  ...
  if (!setup.targets.length && !(entryModel === 'gold-open-ema12' && setup.symbol === 'MGC' && setup.trailingModel === 'gold-five-minute-1r')) {
...
function trackTradeLifecycle(plan, candles, config, options = {}) {
  options = { ...options, flattenAt: options.flattenAt || plan.setup.flattenAt };
  ...
  for (const candle of candles) {
    ...
    if (plan.setup.trailingModel === 'gold-five-minute-1r' && plan.setup.symbol === 'MGC') {
      trailBars.push(candle);
      if (trailBars.length > 5) trailBars.shift();
      // A completed bar can tighten only the NEXT candle's stop.
      stop = require('./gold-open-ema.cjs').trail(trailBars, side, entry, plan.setup.stop, stop);
    }
  }
