...
function trackTradeLifecycle(plan, candles, config, options = {}) {
  ...
  return {
    ...
    remainingContracts,
    markPrice,
    mfePoints: round(maxFavorablePoints, 2),
    maePoints: round(maxAdversePoints, 2),
...
function replayPlan(plan, candles, config) {
  return trackTradeLifecycle(plan, candles, config, { closeOpenAtEnd: true });
}
