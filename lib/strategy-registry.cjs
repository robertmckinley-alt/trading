...
const STRATEGIES = Object.freeze([
  {slug:'mgc-open-ema12',symbol:'MGC',name:'Gold Open EMA12',shortName:'Gold EMA12',paperAccountLabel:'Gold Paper Account',
    strategyFamily:'gold-open-ema',strategyFamilyName:'Gold opening direction',activationTime:'09:35 America/New_York',
    researchStage:'Unvalidated gold paper experiment',evidenceLabel:'Gold-only adaptation; trailing assumptions documented',
    description:'Separate $50,000 Micro Gold account. First five-minute NY opening candle versus EMA12, then completed-candle trailing stop. Gold history required.',
    source:{label:'User-supplied opening EMA video',url:null,license:'Independent implementation',status:'Not an exact replication'}},
  {slug:'nq-vwap-stretch-reversion',name:'VWAP Stretch Reversion',shortName:'VWAP Stretch',paperAccountLabel:'Paper Account J',
    strategyFamily:'vwap-reversion',strategyFamilyName:'VWAP mean reversion',activationTime:'09:44–15:57 America/New_York',
    researchStage:'Unvalidated paper experiment',evidenceLabel:'Video-inspired; execution assumptions documented',
    description:'Long at least 2 ATR below cash-session VWAP. One-minute signals throughout the session. Separate $50k account with a 5% starting-balance drawdown limit.',
    source:{label:'User-supplied Quant Lab video',url:null,license:'Independent implementation',status:'Not an exact replication'}} ,
  {
    slug: 'live-9am-sweep',
...
// Independent hypotheses inspired by public captions, not TradeX proprietary rules.
// Keep these out of the running, version-locked forward experiment until reviewed.
const ORB_EXTENSION_RULES = Object.freeze({
  'nq-15m-orb-exit-1r': Object.freeze({ targetMultiples: Object.freeze([1]), exitAllAtTarget: true }),
  'nq-15m-orb-exit-3r': Object.freeze({ targetMultiples: Object.freeze([3]), exitAllAtTarget: true }),
  'nq-15m-orb-failed-breakout-fade': Object.freeze({
    fade: true, minimumBodyFraction: 0.25, maximumBreakoutWickFraction: 0.25,
    minimumSweepTicks: 1, minimumMidpointRewardRisk: 1, exitAllAtTarget: true
  })
});
const ORB_EXTENSION_VARIANTS = Object.freeze([
  ['nq-15m-orb-exit-1r', 'ORB Test: Full Exit at 1R', 'Baseline close-confirmation entries and structural stops, with the whole position exiting at a planned 1R target.'],
  ['nq-15m-orb-exit-3r', 'ORB Test: Full Exit at 3R', 'Identical entries and stops to the 1R experiment, with the whole position exiting at a planned 3R target.'],
  ['nq-15m-orb-failed-breakout-fade', 'ORB Test: Failed Breakout Fade', 'A completed 15-minute candle sweeps one opening-range edge and closes back inside. Fade toward the midpoint only when planned reward is at least 1R.']
].map(([slug, name, description]) => Object.freeze({
  slug, name, shortName: name.replace('ORB Test: ', 'ORB '),
  strategyFamily: slug.endsWith('-fade') ? 'value-reversion' : 'cash-breakout',
  strategyFamilyName: slug.endsWith('-fade') ? 'Value reversion' : 'Cash-session breakout',
  activationTime: '09:59–11:29 America/New_York',
  researchStage: 'Unvalidated historical candidate', forwardPaperEligible: false,
  evidenceLabel: 'Independent hypothesis; no performance claim', description,
  researchRules: ORB_EXTENSION_RULES[slug],
  source: { label: 'bennnytrades public ORB research themes',
    url: 'https://www.instagram.com/bennnytrades/', license: 'Independent implementation; no proprietary code copied',
    status: 'Backtest-only candidate' }
})));

const PRICE_ACTION_VARIANTS = Object.freeze([
  ['nq-15m-orb-qm-retest', 'QM Confirmed Retest', 'Confirmed swing sweep, opposing structure close, and retest within six five-minute bars.'],
  ['nq-15m-orb-sr-flip', 'ORB Support/Resistance Flip', 'Five-minute opening-range break followed by a separate confirming retest.'],
  ['nq-15m-orb-compression', 'ORB Compression Retest', 'Four tightening bars near the opening-range edge, a body-confirmed break, then a retest.']
].map(([slug, name, description]) => Object.freeze({ slug, name, description, shortName: name,
  strategyFamily: 'price-action-research', strategyFamilyName: 'Independent price-action experiments',
  activationTime: '09:49–11:29 America/New_York', forwardPaperEligible: false,
  researchStage: 'Unvalidated historical candidate', researchRules: { ...require('./price-action-patterns.cjs').RULES, independentPattern: true },
  source: { label: 'User-supplied institutional price-action cheat sheet', url: null,
    license: 'Independent implementation; no source code copied', status: 'Historical validation required' } })));
const BACKTEST_STRATEGIES = Object.freeze([...STRATEGIES.filter(s => !s.symbol || s.symbol === 'NQ'), ...ORB_RESEARCH_VARIANTS, ...ORB_FILTER_VARIANTS, ...STRATEGY_RESEARCH_VARIANTS, ...ORB_EXTENSION_VARIANTS, ...PRICE_ACTION_VARIANTS]);
...
module.exports = {
  BACKTEST_STRATEGIES,
  PRICE_ACTION_VARIANTS,
  ORB_RESEARCH_VARIANTS,
  ORB_FILTER_VARIANTS,
  ORB_FILTER_RULES,
  ORB_EXTENSION_RULES,
  ORB_EXTENSION_VARIANTS,
