...
const LEARNING_VERSION = 'orb-calendar-research-v2';
...
function buildOrbLearningReport(report = {}, options = {}) {
  ...
  const manifest = {
    ...
    researchMode: strategies.every((strategy) => strategy.research?.mode === 'fixed-contract') ? 'fixed-contract' : 'fixed-risk',
    researchSizing: strategies.map((strategy) => ({ slug: strategy.slug, mode: strategy.research?.mode, sizing: strategy.research?.sizing || null })),
    window: { start, end, observedEndExclusive: observedEnd }, selection: 'preceding-calendar-training-net-to-drawdown; slug tie-break',
    gates, costs, riskPerTradeUsd: strategies.length && strategies.every((strategy) => strategy.research?.mode === 'fixed-contract') ? null : config.maxRiskPerTradeUsd ?? report.costs?.riskPerTradeUsd ?? null,
    accountRiskPerTradeUsd: config.maxRiskPerTradeUsd ?? report.costs?.riskPerTradeUsd ?? null,
  ...
  const trials = strategies.map((strategy) => {
    const issues = [];
    if (!['fixed-risk', 'fixed-contract'].includes(strategy.research?.mode) || !Array.isArray(strategy.research?.trades)) issues.push('Independent research trades are missing.');
    const seen = new Set();
    const trades = [];
    for (const trade of strategy.research?.trades || []) {
      ...
      if (strategy.research?.mode === 'fixed-contract' && contracts !== 1) issues.push('One-contract research contains a different quantity.');
    ...
    const trial = {
      ...
      sizingMode: strategy.research?.mode, fixedRisk: metrics(trades, dates), doubledCosts: costKnown ? metrics(trades, dates, extraCost) : null,
