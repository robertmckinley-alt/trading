...
## ORB one-contract correction

`orbResearchVersion: one-contract-v1` replaces risk-capped ORB research with one NQ contract per eligible signal. Dollar risk varies with structural stop width; there is no $500 entry veto or accumulated account loss cutoff. An adverse entry gap does not shrink or reject the quantity for affordability, but entry beyond the structural stop or any target still rejects the order. Replay preserves real stops and targets, one tick of adverse slippage per leg and round-trip commission. A single contract exits in full at its first target. This differs from fractional scale-out or multi-contract strategies. The account simulation and live paper execution retain their existing risk controls. Other strategy research retains fixed-dollar sizing. Every ORB signal records its stop width, fill outcome and P&L for inspection. Old capped results remain labeled until a fresh VPS run completes.
